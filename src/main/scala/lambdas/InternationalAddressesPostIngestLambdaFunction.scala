...
class InternationalAddressesPostIngestLambdaFunction
  ...
  override def handleRequest(input: jMap[String, Object],
    ...
    try {
      Await.ready(doPostIngest(Repository().forIngest, countries), 1.minutes)
    } catch {
  ...
  def doPostIngest(repository: IngestRepository, countries: List[Map[String, String]]): Future[Long] = {
    println(s"Beginning post-ingest processing of international addresses")

    repository.postIngestProcessing(countries).unsafeToFuture()
  }
}
